...
export default function MenuItemCard({ item }: { item: MenuItem }) {
  ...
  return (
    <div className="flex flex-col justify-between rounded-2xl border border-cream-dark bg-white/60 p-5 shadow-sm transition-shadow hover:shadow-md">
      <div>
        <div className="flex items-start justify-between gap-2">
          <h3 className="font-display text-lg font-semibold text-maroon">{item.name}</h3>
          {item.vegetarian && (
        ...
        {item.variants && (
          <p className="mt-1 text-sm text-charcoal/70">{item.variants.join(" · ")}</p>
        )}
        {item.soldOut && (
          <p className="mt-2 text-xs font-semibold uppercase tracking-wide text-terracotta">
            Sold out
          </p>
        )}
      </div>
